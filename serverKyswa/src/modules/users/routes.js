...
function createUsersRoutes(dependencies) {
  ...
  // ─────────────────────────────────────────────────────
  // ROUTES PROFIL (utilisateur connecté) — PLACÉES AVANT /:id
  // ─────────────────────────────────────────────────────

  /**
   * GET /api/users/me ou /api/profile/me
  ...
  router.get('/profile/me', protect, getMeHandler);

  /**
   * PATCH /api/users/me ou /api/profile/me
   * Modifier son propre profil
   */
  const patchMeHandler = async (req, res, next) => {
    try {
      ...
      if (Object.keys(updates).length === 0 && !req.body.email) {
      ...
      return res.status(200).json({ success: true, data: profile, user: { ...profile, etat: profile.actif ? 'ACTIF' : 'INACTIF' }, message: 'Profil mis à jour' });
    } catch (e) { next(e); }
  };

  router.patch('/me', protect, patchMeHandler);
  router.patch('/profile/me', protect, patchMeHandler);

  /**
   * PATCH /api/users/me/password ou /api/profile/me/password
   * Changer son mot de passe
   */
  const patchPasswordHandler = async (req, res, next) => {
    try {
