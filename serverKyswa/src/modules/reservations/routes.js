...
function createReservationsRoutes(dependencies) {
  ...
  router.post('/', protect, checkPermission('reservations', 'create'), async (req, res, next) => {
    try {
      ...
      const departId = data.packageKId || data.departId || data.depart_id || null;
      const agentId = req.user?.id || null;

      // Déterminer le service depuis le départ si disponible
      let service = data.service;
      if (!service && departId) {
        try {
          const dep = await prismaClient.departs.findUnique({ where: { id: departId } });
          if (dep && dep.service) service = dep.service;
        } catch (_) {}
      }
      if (!['Oumra', 'Hajj', 'Ziara Fès', 'Billet Avion', 'Assurance', 'Location Voiture'].includes(service)) {
        service = 'Oumra';
      }

      // Normaliser formule selon inscriptions_formule_check ('VIP', 'Confort', 'Eco', 'Standard')
      let formule = data.formule || data.niveauConfort || null;
      if (formule) {
        const fUpper = String(formule).toUpperCase();
        if (fUpper === 'ECO') formule = 'Eco';
        else if (fUpper === 'CONFORT') formule = 'Confort';
        else if (fUpper === 'VIP') formule = 'VIP';
        else if (fUpper === 'STANDARD') formule = 'Standard';
        else if (!['VIP', 'Confort', 'Eco', 'Standard'].includes(formule)) formule = null;
      }

      // Normaliser statut_client selon inscriptions_statut_client_check
      let statutClient = data.statutClient || data.statut_client || 'Inscrit';
      const sUpper = String(statutClient).toUpperCase();
      if (sUpper === 'INSCRIT') statutClient = 'Inscrit';
      else if (sUpper === 'CONFIRME' || sUpper === 'CONFIRMÉ') statutClient = 'Confirmé';
      else if (sUpper === 'PARTI') statutClient = 'Parti';
      else if (sUpper === 'RENTRE' || sUpper === 'RENTRÉ') statutClient = 'Rentré';
      else if (sUpper === 'DESISTE' || sUpper === 'DÉSISTÉ') statutClient = 'Désisté';
      else if (sUpper === 'REMBOURSE' || sUpper === 'REMBOURSÉ') statutClient = 'Remboursé';
      else if (!['Inscrit', 'Confirmé', 'Visa obtenu', 'Parti', 'Rentré', 'Désisté', 'Remboursé'].includes(statutClient)) statutClient = 'Inscrit';

      // Normaliser type_chambre selon inscriptions_type_chambre_check
      let typeChambre = data.type_chambre || data.typeChambre || null;
      if (typeChambre) {
        const tUpper = String(typeChambre).toUpperCase();
        if (tUpper === 'SINGLE') typeChambre = 'Single';
        else if (tUpper === 'DOUBLE') typeChambre = 'Double';
        else if (tUpper === 'TRIPLE') typeChambre = 'Triple';
        else if (tUpper === 'QUADRUPLE') typeChambre = 'Quadruple';
        else if (!['Single', 'Double', 'Triple', 'Quadruple'].includes(typeChambre)) typeChambre = null;
      }

      const clientIds = Array.isArray(data.clients) && data.clients.length > 0
        ? data.clients
        : (data.clientId || data.client_id ? [data.clientId || data.client_id] : [null]);
      ...
      for (const clientId of clientIds) {
        ...
        createdItems.push(fullItem || item);
      }

      const mainItem = createdItems[0];

      res.status(201).json({ 
        success: true, 
        data: mainItem, 
        reservation: mainItem,
        items: createdItems,
