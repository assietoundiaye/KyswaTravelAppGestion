  ...
  if (!this.numero) {
    ...
    while (tentative < 10) {
      try {
        ...
        // Rareté: collision de numéro, réessayer
        console.warn(`Collision numéro INS, tentative ${tentative + 1}`);
        tentative++;
      }
    }
    if (!this.numero) {
      // Générer un numéro unique avec timestamp en fallback
      this.numero = `INS-${new Date().getFullYear()}-${Date.now().toString().slice(-4)}`;
    }
  }
