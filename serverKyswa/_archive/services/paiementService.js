...
async function payerReservation(reservationId, { montant, dateReglement, mode, reference }, userId) {
  ...
  if (reservation.clients && reservation.clients.length > 0) {
    const premierClient = await reservation.populate({
      path: 'clients',
      select: 'nom prenom email',
    });
    
    if (premierClient.clients[0]?.email) {
      const totalPaye = reservation.paiements.reduce(
        (s, p) => s + (p.montant ? parseFloat(p.montant.toString()) : 0),
        0
      );
      const soldeRestant = (reservation.montantTotalDu || 0) - totalPaye;
      
      emailService.envoyerConfirmationPaiement(paiement, premierClient.clients[0], soldeRestant)
        .catch(err => console.error('Erreur envoi email paiement:', err.message));
    }
  }
