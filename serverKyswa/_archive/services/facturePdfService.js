...
/**
 * Génère une facture PDF pour une commande shop
 */
function buildShopOrderFacturePdf({ order }) {
  const montantTotal = Number(order.montantTotal || 0);
  const montantPaye = order.payment ? Number(order.payment.montant || 0) : 0;
  const factureNum = `SHOP-${order.orderNumber || order._id.toString().slice(-6)}`;
  const dateStr = fmtDate(order.createdAt);
  const clientNom = order.clientId ? `${order.clientId.nom} ${order.clientId.prenom}` : '—';
  ...
  doc.setFontSize(14);
  doc.setTextColor(...GREEN);
  doc.text('FACTURE SHOP', W / 2, y + 6, { align: 'center' });
  doc.setDrawColor(...GREEN);
  doc.setLineWidth(0.5);
  doc.line(60, y + 9, W - 60, y + 9);
  y += 16;
  ...
  // Bloc client
  doc.setFillColor(...GRAY_LIGHT);
  doc.rect(block1X, y, blockW, 32, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(...GREEN);
  doc.text('CLIENT', block1X + 4, y + 6);
  ...
  let cy = y + 13;
  if (order.clientId) {
    doc.text(`Nom : ${order.clientId.nom || '—'}`, block1X + 4, cy); cy += 5;
    doc.text(`Prénom : ${order.clientId.prenom || '—'}`, block1X + 4, cy); cy += 5;
    doc.text(`Tél : ${order.clientId.telephone || '—'}`, block1X + 4, cy);
  }

  // Bloc commande
  doc.setFillColor(...GRAY_LIGHT);
  doc.rect(block2X, y, blockW, 32, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(...GREEN);
  doc.text('COMMANDE', block2X + 4, y + 6);
  doc.setDrawColor(...GREEN);
  doc.setLineWidth(0.4);
  doc.line(block2X + 4, y + 7.5, block2X + 30, y + 7.5);
  
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8.5);
  doc.setTextColor(...BLACK);
  let oy = y + 13;
  doc.text(`N° : ${order.orderNumber || '—'}`, block2X + 4, oy); oy += 5;
  doc.text(`Date : ${fmtDate(order.createdAt)}`, block2X + 4, oy); oy += 5;
  doc.text(`Statut : ${order.status === 'PAYE' ? 'Payée' : order.status === 'EN_ATTENTE_PAIEMENT' ? 'En attente' : order.status}`, block2X + 4, oy);
  y += 38;
  ...
  doc.setFontSize(9);
  doc.setTextColor(...GREEN);
  doc.text('Articles commandés :', 14, y);
  y += 6;

  // En-têtes du tableau des articles
  const colWidths = [70, 20, 30, 30, 36];
  const tableX = 14;
  const rowH = 7;
  const headers = ['Produit', 'Qté', 'Prix unit.', 'Sous-total', 'Total'];
  ...
  items.forEach((item, ri) => {
    ...
    // Nom du produit (avec troncature si nécessaire)
    const nomProduit = item.nomProduit || 'Produit inconnu';
    const nomTronque = doc.splitTextToSize(nomProduit, colWidths[0] - 6);
    doc.text(nomTronque[0] + (nomTronque.length > 1 ? '...' : ''), tableX + 3, y + 5);
    
    // Quantité
    doc.text(String(item.quantite || 0), tableX + colWidths[0] + colWidths[1] - 3, y + 5, { align: 'right' });
    
    // Prix unitaire
    doc.text(fmtMoney(item.prixUnitaire || 0), tableX + colWidths[0] + colWidths[1] + colWidths[2] - 3, y + 5, { align: 'right' });
    
    // Sous-total
    doc.text(fmtMoney(item.sousTotal || 0), tableX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] - 3, y + 5, { align: 'right' });
    
    // Total (même que sous-total pour les articles individuels)
    doc.text(fmtMoney(item.sousTotal || 0), tableX + colWidths.reduce((a, b) => a + b, 0) - 3, y + 5, { align: 'right' });
  ...
  if (items.length === 0) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...GRAY_TEXT);
    doc.text('Aucun article dans cette commande', W / 2, y + 15, { align: 'center' });
    y += 25;
  }
  ...
  if (order.payment) {
    ...
    const paymentRows = [[
      '1',
      fmtDate(order.payment.dateReglement || order.paidAt),
      order.payment.mode || '—',
      order.payment.reference || '—',
      fmtMoney(order.payment.montant || 0)
    ]];

    y = drawPaymentsTable(doc, y, paymentRows);
  } else {
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(8);
    doc.setTextColor(...GRAY_TEXT);
    doc.text('Commande en attente de paiement', 14, y);
    y += 8;
  }

  // Totaux
  y += 10;
  y = drawTotals(doc, y, montantTotal, montantPaye, montantTotal - montantPaye);

  // Signature
  y += 15;
  const sigBlockW = (W - 34) / 2;
  const sig1X = 14;
  const sig2X = 14 + sigBlockW + 6;

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(...GREEN);
  doc.text('Signature Responsable Shop', sig1X + sigBlockW / 2, y, { align: 'center' });
  doc.text('Signature Client', sig2X + sigBlockW / 2, y, { align: 'center' });
  y += 4;

  doc.setDrawColor(...GRAY_TEXT);
  doc.setLineWidth(0.3);
  doc.rect(sig1X, y, sigBlockW, 25, 'S');
  doc.rect(sig2X, y, sigBlockW, 25, 'S');

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(...GRAY_TEXT);
  doc.text(clientNom || '', sig2X + sigBlockW / 2, y + 29, { align: 'center' });

  drawFooter(doc);

  const buffer = doc.output('arraybuffer');
  return { buffer: Buffer.from(buffer), factureNum };
}